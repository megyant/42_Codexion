...
/*   Updated: 2026/04/24 11:48:42 by mbotelho         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	*coder_routine(void *data)
{
	...
	coder->last_compile_time = coder->workspace->start_simulation;
...
void	bubble_down(t_dongle *dongle, int i)
{
	...
	while (1)
	{
		...
		if (left < dongle->queue.size
			&& (dongle->queue.heap[left].priority_value < dongle->queue.heap[smallest].priority_value))
			smallest = left;
		if (right < dongle->queue.size
			&& (dongle->queue.heap[right].priority_value < dongle->queue.heap[smallest].priority_value))
			smallest = right;
		if (smallest == i)
			break ;
...
void	bubble_up(t_dongle *dongle, int i)
{
	...
	{
		parent_thread = (i - 1) / 2;
		if (dongle->queue.heap[i].priority_value >= dongle->queue.heap[parent_thread].priority_value)
			break ;
		temp = dongle->queue.heap[i];
...
int	grab_dongles(t_coder *coder)
{
	...
	while (get_sim_status(coder->workspace))
	{
		if (!request_dongle(coder, first))
			return (0);
		if (try_request_dongle(coder, second))
			return (1);
...
int	try_request_dongle(t_coder *coder, t_dongle *dongle)
{
	...
	success = 0;
	safe_mutex_handle(&dongle->mutex, LOCK, coder->workspace);
	if (dongle->current_user == -1 && (dongle->queue.size == 0
	|| heap_peek(dongle) == coder->id))
	{
		if (dongle->queue.size == 0)
			queue_management(coder, dongle);
		dongle->current_user = coder->id;
